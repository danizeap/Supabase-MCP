...
/**
 * Shape of the error object returned by supabase-js Storage calls. Different operations
 * expose the HTTP status on different fields, so we check several.
 */
...
function statusOf(err: SupabaseLikeError): number | undefined {
...
export function mapStorageError(
  err: unknown,
  context?: { bucket?: string; path?: string },
): string {
