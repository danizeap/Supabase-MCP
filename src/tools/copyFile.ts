...
export default defineTool({
  ...
  title: "Copy a file",
  description:
    "Make a copy of a file somewhere else — in the same bucket, or in a DIFFERENT bucket (set " +
  ...
  async run(args): Promise<string> {
    ...
    if (from === to && !crossBucket) {
      throw new FriendlyError(
        "The copy would land on top of the original — give the copy a different name or a different bucket.",
      );
    }
    ...
    if (await objectExists(supabase, destBucket, to)) {
      throw new FriendlyError(
        `A file already exists at "${to}" in bucket "${destBucket}". Choose a different name — this plugin never overwrites existing files.`,
      );
    }
