...
export default defineTool({
  ...
  description:
    `Create a new Supabase Storage bucket. ${BUCKET_NAME_HELP} ` +
  ...
  inputSchema: {
    name: z
      .string()
      .describe(
        'Bucket name. Must match the convention, e.g. "marketing-q3-launch" or "brand-logos".',
      ),
    public: z
      .boolean()
      .default(false)
      .describe(
        "true = anyone with the URL can view files (for openly shared assets). false (default) = private; files need a signed link.",
      ),
    allowed_mime_types: z
      .array(z.string())
      .optional()
      .describe(
        'Optional list of allowed file types, e.g. ["image/*", "application/pdf"]. Defaults to common media types.',
      ),
  ...
  async run(args): Promise<string> {
    ...
    const allowedMime =
      args.allowed_mime_types && args.allowed_mime_types.length > 0
        ? args.allowed_mime_types
        : [...DEFAULT_ALLOWED_MIME_TYPES];

    const sizeLimitBytes =
      args.file_size_limit_mb != null
