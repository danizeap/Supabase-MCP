...
async function getBucketConstraints(
  bucket: string,
): Promise<{ constraints: BucketConstraints; isPublic: boolean }> {
  ...
  return {
    ...
    constraints: {
      name: bucket,
      allowedMimeTypes: (data.allowed_mime_types as string[] | null) ?? null,
      fileSizeLimitBytes:
...
export default defineTool({
  ...
  title: "Upload a file",
  description:
    "Upload a local file (image, video, audio, PDF, design asset) to a bucket. The file type and " +
  ...
  inputSchema: {
    ...
    destination_path: z
      ...
      .optional()
      .describe(
        "Where to store it inside the bucket. Omit to use the file's own name. End with '/' to place it in a folder, e.g. 'logos/' stores it as logos/<filename>.",
      ),
    content_type: z
      .string()
      .optional()
  ...
  async run(args): Promise<string> {
    ...
    try {
      ...
      throw new FriendlyError(
        `Couldn't find a file at "${args.local_file_path}". Double-check the path (including drive letter and slashes) and try again.`,
      );
    }
    if (!fileStat.isFile()) {
      throw new FriendlyError(
        `"${args.local_file_path}" is a folder, not a file. Point to a single file to upload.`,
      );
    }
    ...
    if (destPath.endsWith("/")) destPath = `${destPath}${fileName}`;
    ...
    const base =
      `Uploaded "${fileName}" (${formatBytes(fileStat.size)}) to bucket "${args.bucket}".\n` +
      `• Stored at: ${stored}`;
    if (isPublic) {
