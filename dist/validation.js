...
export function validateBucketName(name) {
    if (!BUCKET_NAME_PATTERN.test(name)) {
        throw new FriendlyError(`"${name}" isn't a valid bucket name. ${BUCKET_NAME_HELP}`);
    }
}
 ...
 */
export function mimeAllowed(mime, allowed) {
    if (!allowed || allowed.length === 0)
        return true; // null/empty => no restriction
 ...
 */
export function validateUpload(file, bucket) {
    if (!mimeAllowed(file.mime, bucket.allowedMimeTypes)) {
        const allowedList = bucket.allowedMimeTypes?.join(", ") || "any type";
        throw new FriendlyError(`This file looks like "${file.mime}", which isn't allowed in bucket "${bucket.name}". That bucket accepts: ${allowedList}. Use a different file type or a different bucket.`);
    }
    if (bucket.fileSizeLimitBytes != null &&
        bucket.fileSizeLimitBytes > 0 &&
        file.sizeBytes > bucket.fileSizeLimitBytes) {
        throw new FriendlyError(`This file is ${formatBytes(file.sizeBytes)}, but bucket "${bucket.name}" only allows files up to ${formatBytes(bucket.fileSizeLimitBytes)}. Use a smaller file, or ask whoever set up the plugin to raise the bucket's limit.`);
    }
}
