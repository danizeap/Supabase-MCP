...
async function destinationExists(bucket, path) {
...
export default defineTool({
    ...
    description: "Move or rename a file within a bucket (for example, tidy 'draft.png' into 'logos/final.png'). " +
    ...
    async run(args) {
        ...
        if (await destinationExists(args.bucket, to)) {
            throw new FriendlyError(`A file already exists at "${to}" in bucket "${args.bucket}". Choose a different name — this plugin never overwrites existing files.`);
        }
        const supabase = getSupabase();
