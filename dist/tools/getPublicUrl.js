...
export default defineTool({
    ...
    description: "Get the permanent public web link for a file. This only works for PUBLIC buckets — files anyone " +
    ...
    async run(args) {
        ...
        if (!bucket.public) {
            throw new FriendlyError(`Bucket "${args.bucket}" is private, so it has no public link. Make a temporary shareable link instead with "get_signed_url", or (if these files are meant to be public) create a public bucket.`);
        }
