...
export default defineTool({
    ...
    description: "List all Supabase Storage buckets with their settings: whether each is public or private, its file size limit, and which file types it accepts. Use this first when someone asks 'where can I upload this?' or before creating a new bucket, so you don't duplicate one that already exists.",
    inputSchema: {},
    async run() {
        const supabase = getSupabase();
        const { data, error } = await supabase.storage.listBuckets();
        if (error)
            throw new FriendlyError(toFriendlyMessage(error));
        ...
        const lines = data.map((b) => {
            ...
            const size = b.file_size_limit != null && b.file_size_limit > 0
                ? formatBytes(b.file_size_limit)
                : "no limit";
            const types = Array.isArray(b.allowed_mime_types) && b.allowed_mime_types.length > 0
